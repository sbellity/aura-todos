define({
  ...
  View: {
    events: {
      'click [data-task-action]':  function(e) {
        ...
        var task = this.component.tasks[data.taskId];
  ...
  initialize: function() {
    ...
    _.each(['add', 'destroy', 'toggle'], function(action) {
      self.sandbox.on('tasks.' + action, self.handleTaskAction, self);
    });
    this.sandbox.on('tasks.clear', this.clearCompleted, this);
    this.render();
  },

  render: function() {
    this.html(this.renderTemplate('list'));
  ...
  handleTaskAction: function(task, action) {
    ...
    if (fn) {
      ...
      fn && fn.call(this, task);
      this.getStats();        
    }
  },

  addTask: function(task) {
    var view = this.taskViews[task.id] = $(this.renderTemplate('item', task));
